...
- **关键词管理** — 文字输入 / 语音识别，支持批量添加
- **AI 分析** — 调用 Claude / OpenAI / 自定义模型，流式生成思维导图和方案
- **思维导图** — 交互式画布，可拖拽、编辑节点、添加关联、锁定节点
- **关键词诊断** — AI 自动识别相近词、矛盾词、缺失维度，一键清理
- **推导方案** — AI 生成多个可行方案卡片，支持详细撰写与对话修改
- **笔记 Note** — 补充背景要求，随关键词一起发给 AI
- **锁定机制** — 锁定满意的节点或方案，重新分析时 AI 保留并在此基础上优化
- **导出** — Markdown / Word(.docx) / 思维导图图片(.png)
...
**Windows 用户**也可以直接双击根目录的 `start.bat`，会自动检查环境、安装依赖并启动。
...
- **API Key**：填入对应的密钥
- **Model**：推荐 `deepseek-v4-flash`或 `gpt-4o`（OpenAI）

API Key 仅保存在**本地浏览器**，不会上传到任何服务器。
...
**锁定功能：** 对满意的节点或方案点击锁定（🔒），再次点击「AI 分析关联」时，AI 会在锁定内容基础上优化，而不是全量覆盖。
...
| 思维导图 | ReactFlow 11 |
| 布局算法 | d3-force |
| AI 接入 | Anthropic SDK / OpenAI SDK |
| 文档导出 | docx + html-to-image |
| 图标 | Lucide React |
...
[MIT](LICENSE) © 2026 BrainSpark Contributors
